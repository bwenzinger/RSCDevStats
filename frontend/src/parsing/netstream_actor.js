var Car = function (id) {
  ...
  this.update = function (frame, actorState) {
    if (
      actorState.properties["TAGame.RBActor_TA:ReplicatedRBState"] === undefined
    ) {
      return
    }

    if (
      actorState.properties["Engine.Pawn:PlayerReplicationInfo"] !== undefined
    ) {
      this.player = frame.get_parsed_actor(
        actorState.properties["Engine.Pawn:PlayerReplicationInfo"][1]
      )
      if (this.player != null) this.player.car = this
    }

    this.position =
      actorState.properties["TAGame.RBActor_TA:ReplicatedRBState"].position
    this.rotation =
      actorState.properties["TAGame.RBActor_TA:ReplicatedRBState"].rotation
    this.driving = actorState.properties["TAGame.Vehicle_TA:bDriving"]

    if (
      !actorState.properties["TAGame.RBActor_TA:ReplicatedRBState"].sleeping
    ) {
      this.velocity =
...
var CarComponentBoost = function (id) {
  ...
  this.update = function (frame, actorState) {
    if (actorState.properties === undefined) return

    if (actorState.properties["TAGame.CarComponent_TA:Vehicle"] !== undefined) {
      this.car = frame.get_parsed_actor(
        actorState.properties["TAGame.CarComponent_TA:Vehicle"][1]
      )
      this.car.components["boost"] = this
    }

    if (
      actorState.properties["TAGame.CarComponent_TA:ReplicatedActive"] !==
      undefined
    ) {
      this.active =
        actorState.properties["TAGame.CarComponent_TA:ReplicatedActive"] % 2 !=
        0
    }
...
var Player = function (id) {
  ...
  this.getSystemForType = function (type) {
    if (type == 0) return "SplitScreen"
    else if (type == 1) return "Steam"
    else if (type == 2) return "PS4"

    return "Unknown"
  }

  this.require = function (propertyName) {
    if (this.actorState.properties[propertyName] === undefined) return false
  ...
  this.update = function (frame, actorState) {
    ...
    if (
      !this.require("Engine.PlayerReplicationInfo:PlayerName") ||
      !this.require("Engine.PlayerReplicationInfo:Team") ||
      !this.require("Engine.PlayerReplicationInfo:UniqueId")
    ) {
      return
    }

    this.team = frame.get_parsed_actor(
      actorState.properties["Engine.PlayerReplicationInfo:Team"][1]
    )
    ...
    this.systemType = this.getSystemForType(
      actorState.properties["Engine.PlayerReplicationInfo:UniqueId"][0]
    )
    ...
    if (this.require("Engine.PlayerReplicationInfo:bReadyToPlay")) {
      this.readyToPlay =
        actorState.properties["Engine.PlayerReplicationInfo:bReadyToPlay"]
    }

    if (this.require("TAGame.PRI_TA:ReplicatedGameEvent")) {
      this.gameEvent = frame.get_parsed_actor(
        actorState.properties["TAGame.PRI_TA:ReplicatedGameEvent"][1]
      )
    }
...
var Ball = function (id) {
  ...
  this.update = function (frame, actorState) {
    ...
    if (actorState.properties["TAGame.Ball_TA:GameEvent"] !== undefined) {
      this.gameEvent = frame.get_parsed_actor(
        actorState.properties["TAGame.Ball_TA:GameEvent"][1]
      )
    }
...
var Team = function (id) {
  ...
  this.update = function (frame, actorState) {
    ...
    if (actorState.properties["TAGame.Team_TA:GameEvent"] !== undefined) {
      this.gameEvent = frame.get_parsed_actor(
        actorState.properties["TAGame.Team_TA:GameEvent"][1]
      )
    }
