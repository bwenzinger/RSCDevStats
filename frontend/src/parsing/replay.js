...
import PropertyMapper from "./netstream_property"
import ActorMapper from "./netstream_actor"
...
class NetStreamReader {
  ...
  readSerializedInt(max_value) {
    if (max_value === undefined) {
      throw new Error(
        "Undefined max value when calling readSerializedInt in NetStreamReader."
      )
    }
  ...
  readSerializedFloat(max_value, numbits) {
    ...
    if (max_value > max_bit_value) {
      ...
      var inv_scale2 = 1.0 / scale
      value = unscaled_value * inv_scale2
    }
  ...
  readByteVector() {
    ...
    if (this.readBool()) x = this.readByte()

    if (this.readBool()) y = this.readByte()

    if (this.readBool()) z = this.readByte()
  ...
  readStringUTF8(length) {
    var bytes = []
    for (var i = 0; i < length; i++) bytes.push(this.readByte())
  ...
  readStringUTF16(length) {
    var bytes = []
    for (var i = 0; i < length; i += 2) bytes.push(this.readBits(16))

    var str = ""
    for (var i2 = 0; i < bytes.length; i2++) {
      str += String.fromCharCode(bytes[i2])
    }
  ...
  peakBits(num, format, formatType) {
    ...
    if (format) {
      //   if (formatType == "hex") {
      //     str = ""
      //     for (var i2 = 0; i2 < num; i2 += 8) {
      //       var byte = this.internal._view.getBitsUnsigned(
      //         this.internal._index + i2,
      //         8
      //       )
  ...
  parse() {
    ...
    if (alive) {
      ...
      if (is_new) {
        ...
        this.class_name = this.replay.convert_arch_type_to_class_name(
          this.type_name
        )
        this.properties = {}
        ...
        if (
          this.type_name.indexOf("Archetypes.Ball") != -1 ||
          this.type_name.indexOf("Car_Default") != -1
        )
        ...
        while (this.stream.readBool()) {
          var prop_id = this.stream.readSerializedInt(
            cache.get_max_property_id() + 1
          )
          var prop_name = this.replay.object[cache.get_property(prop_id)]

          if (PropertyMapper[prop_name] === undefined) {
            throw new Error(
              "No mapping available for property '" + prop_name + "'."
            )
          }
...
class NetStreamFrame {
  ...
  get_actor(id) {
    if (this.actor[id] === undefined) return null

    return this.actor[id]
  }

  get_parsed_actor(id) {
    if (this.parsed_actor[id] === undefined) return null
  ...
  add_actor(state) {
    if (this.actor[state.id] !== undefined) return
  ...
  parse() {
    ...
    while (true) {
      ...
      if (!actorPresent) break

      var state = new ActorState(this.replay, this.stream, this.alive_buffer)
      state.parse()

      if (state.state == ActorState.DEAD) this.add_actor(state)
    }
    ...
    for (var key2 in this.parsed_actor) {
      this.parsed_actor[key2].parsed.update(this, this.parsed_actor[key2])
    }
  }
}
...
class NetCache {
  ...
  set_max_property_id(id) {
    if (id <= this.max_property_id) return
...
export class Replay {
  ...
  load(options) {
    ...
    if (this.loadInProgress !== null) return this.loadInProgress
    ...
    this.asyncCall(function () {
      if (typeof this.file === "string") {
        var callback = function (err, buffer) {
          ...
          this.parse(buffer, options).then(
            function () {
              this.loaded = true
              this.loadInProgress = null
        ...
        this.parse(this.file, options).then(
          function () {
            this.loaded = true
            this.loadInProgress = null
            defer.resolve()
  ...
  parse(buffer, options) {
    ...
    this.asyncCall(function () {
      ...
      for (var i2 = 0; i2 < data.object.length; i2++)
        this.object.push(data.object[i2].string)
      for (var i3 = 0; i3 < data.name.length; i3++)
        this.name.push(data.name[i3].string)
      for (var i4 = 0; i4 < data.package.length; i4++)
        this.package.push(data.package[i4].string)

      for (var i5 = 0; i5 < data.tickmark.length; i5++) {
        var mark = data.tickmark[i5]
  ...
  parse_property(property) {
    if (property.more.type == "ArrayProperty") {
      ...
      for (var j = 0; j < property.more.details.array.length; j++) {
        ...
        for (var k = 0; k < property.more.details.array[j].part.length; k++) {
          ...
          if (sub_property.name == "None") continue
    ...
    if (property.more.type == "StrProperty") {
      ...
      else str = property.more.details.value.toString("utf8")
  ...
  parse_properties() {
    for (var i = 0; i < this.data.properties.length; i++) {
      ...
      if (property.name == "None") continue
  ...
  parse_netcache() {
    ...
    temp_cache.reverse()
    for (var i2 = 0; i2 < temp_cache.length - 1; i2++) {
      var cache2 = temp_cache[i2]
      var j = i + 1

      while (j < temp_cache.length) {
        var item = temp_cache[j]
        if (item.id == cache2.parent_id) {
          item.add_child(cache2)
          break
        } else {
          j++
        }
  ...
  convert_arch_type_to_class_name(arch_type) {
    ...
    else if (arch_type.indexOf("Archetypes.Ball") != -1) return "TAGame.Ball_TA"
    else {
      var name = arch_type
        .replace(/_\d+/, "")
        .split(".")
